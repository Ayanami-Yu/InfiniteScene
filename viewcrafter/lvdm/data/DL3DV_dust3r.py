...
class WebVid(Dataset):
    ...
    def _get_video_path(self, sample):
        full_video_fp = os.path.join(
            self.data_dir,
            sample["oripath"][1:] if sample["oripath"][0] == "/" else sample["oripath"],
        )
        cond_full_video_fp = os.path.join(
            self.data_dir,
            (
                sample["videopath"][1:]
                if sample["videopath"][0] == "/"
                else sample["videopath"]
            ),
        )
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            try:
                ...
                if (
                    len(video_reader) < self.video_length
                    or len(cond_video_reader) < self.video_length
                ):
                    print(
                        f"video length ({len(video_reader)}) or Cond video length ({len(cond_video_reader)}) is smaller than target length({self.video_length})"
                    )
                    index += 1
                    continue
                else:
        ...
        assert (
            frames_cond.shape[0] == self.video_length
        ), f"{len(frames_cond)}, self.video_length={self.video_length}"
        frames = (
            torch.tensor(frames.asnumpy()).permute(3, 0, 1, 2).float()
        )  # [t,h,w,c] -> [c,t,h,w]
        frames_cond = (
            torch.tensor(frames_cond.asnumpy()).permute(3, 0, 1, 2).float()
        )  # [t,h,w,c] -> [c,t,h,w]
