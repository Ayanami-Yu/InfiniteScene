...
class LucidDreamer:
    ...
    def run(
        ...
        gallery, depth = self.render_video_preset(
            render_camerapath, example_name=example_name
        )
    ...
    def generate_pcd(
        ...
        internal_render_poses = get_pcdGenPoses(
            "hemisphere", {"center_depth": center_depth}, deg_denom=angle_scale
        )
    ...
    def resize_image(self, rgb_cond, prompt=None, negative_prompt="", generator=None):
        ...
        if w_in / h_in > 1.1 or h_in / w_in > 1.1:
            assert (
                prompt is not None
            ), "Irregular-sized image requires prompt to do inpainting"
