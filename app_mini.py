...
if len(glob.glob(os.path.join(d, "*.ply"))) < 8:
    snapshot_download(
        repo_id="ironjr/LucidDreamerDemo",
        repo_type="model",
        local_dir=d,
...
try:
    ...
    subprocess.run(
        shlex.split(
            f"pip install {root}/submodules/wheels/simple_knn-0.0.0-cp39-cp39-linux_x86_64.whl"
        )
    )
try:
    ...
    subprocess.run(
        shlex.split(
            f"pip install {root}/submodules/wheels/depth_diff_gaussian_rasterization_min-0.0.0-cp39-cp39-linux_x86_64.whl"
        )
    )
...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Row():
        ...
        result_gallery = gr.Video(
            label="RGB Video", show_label=True, autoplay=True, format="mp4"
        )

        result_depth = gr.Video(
            label="Depth Video", show_label=True, autoplay=True, format="mp4"
        )
    ...
    with gr.Row():
        ...
        with gr.Column():
            run_button = gr.Button(
                value="Run! (it may take a while)", elem_id="run-button"
            )
