...
def normalize_depth(
    ...
    if pre_clip_max and pre_clip_max > 0 and len(val_dpt) > 0:
        val_dpt = val_dpt.clip(0.0, pre_clip_max)
...
def normalize_depth_torch(
    ...
    # do not clip if pre_clip_max is not provided
    if pre_clip_max and pre_clip_max > 0 and len(val_dpt) > 0:
        val_dpt = torch.clamp(val_dpt, 0.0, pre_clip_max)

    if len(val_dpt) > 0:
        dpt_min = torch.quantile(val_dpt, lower_thresh / 100)
    else:
        dpt_min = 0.0

    if len(val_dpt) > 0:
        dpt_max = torch.quantile(val_dpt, upper_thresh / 100)
    else:
        dpt_max = 0.0
    ...
    sdpt = torch.clamp(sdpt, dpt_min, dpt_max)
    sdpt_norm = (sdpt - dpt_min) / (dpt_max - dpt_min)
    return sdpt_norm, dpt_min, dpt_max


def interp_depth(sdpt, k=3, w_dist=10.0, lb=0.0):
    h, w = sdpt.shape

    if (sdpt <= lb).all():
        return np.ones((h, w)) * lb, np.zeros((h, w))
    ...
    if k == 1:
        dpt[inval_x * w + inval_y] = sdpt.reshape(
            -1,
        )[val_pos[inds][..., 0] * w + val_pos[inds][..., 1]]
    else:
    ...
    dist_map = cv2.distanceTransform(
        (1 - val_msk).astype(np.uint8), distanceType=cv2.DIST_L2, maskSize=5
    )
