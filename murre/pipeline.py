...
class MurrePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        duplicated_idpt = (
            idpt.unsqueeze(0).unsqueeze(0).expand(ensemble_size, 3, -1, -1)
        )
        duplicated_dist = (
            dist.unsqueeze(0).unsqueeze(0).expand(ensemble_size, -1, -1, -1)
        )
        single_rgb_dataset = TensorDataset(
            duplicated_rgb, duplicated_idpt, duplicated_dist
        )
        if batch_size > 0:
            _bs = batch_size
        else:
    ...
    def single_infer(
        ...
        dist_down = F.interpolate(
            dist_in, size=(rgb_latent.shape[2], rgb_latent.shape[3]), mode="nearest"
        )
