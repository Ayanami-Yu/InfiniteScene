...
class ZoomStack:
    ...
    def downscale(self, image, level_diff):
        """
        level_diff: the difference of two zoom-in levels
        """
        image_c = image.clone()
        p_level = self.p_levels[level_diff]
        kernel_size = max(
            3, min((p_level // 2) * 2 + 1, image.shape[-2], image.shape[-1]) | 1
        )
        sigma = p_level / 2
        # prefilter the image with a truncated Gaussian kernel of size p_i x p_i
        image_c = TF.gaussian_blur(image_c, kernel_size=kernel_size, sigma=sigma)
        # resample with a stride of p_i
        return image_c[:, ::p_level, ::p_level]

    def downscale_and_pad(self, image, level_diff):
        """
        Downscales the image by factor level_diff, and zero-pads the image back to size H x W.
        """
    ...
    def render_image(self, i):
        ...
        for level_diff in range(1, max_level_diff + 1):
            ...
            image = mask * d_image + (1 - mask) * image  # alg.1 line 3
        return image
    ...
    def render_noise(self, i):
        ...
        for level_diff in range(1, max_level_diff + 1):
            noise_image, mask = self.downscale_and_pad(
                self.E[i + level_diff], level_diff
            )
            noise = (self.p_levels[level_diff]) * mask * noise_image + (
                1 - mask
            ) * noise  # alg.1 line 4  # level_diff = p_j / p_i
        return noise
    ...
    def laplacian_pyramid(self, image, levels):
        ...
        # obtain downscaled versions of `image` at every level smaller than `levels`
    ...
    def reconstruct_from_pyramid(self, pyramids):
        ...
        for laplacian in reversed(pyramids[:-1]):
            image = TF.resize(  # upscale by one level
                image, laplacian.shape[-2:], interpolation=PIL.Image.BILINEAR
            )
            image += laplacian
        return image
        ...
        for i in range(len(self.L)):
            if method == "laplacian":
                ...
                for level_diff in range(depth):
                    ...
                    pyramid = self.laplacian_pyramid(
                        image_crop, levels=(depth - level_diff)
                    )
