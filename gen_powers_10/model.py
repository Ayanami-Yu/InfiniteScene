...
class GenPowers10Pipeline(nn.Module):
    def __init__(self, version):
        ...
        self.scheduler = DDPMScheduler.from_pretrained(
            model_key_1, subfolder="scheduler", torch_dtype=torch.float16
        )
        ...
        self.text_encoder = T5EncoderModel.from_pretrained(
            "DeepFloyd/t5-v1_1-xxl", torch_dtype=torch.float16
        ).requires_grad_(False)
        self.unet = UNet2DConditionModel.from_pretrained(
            model_key_1, subfolder="unet", torch_dtype=torch.float16, variant="fp16"
        ).requires_grad_(False)

        self.stage_2_pipe = IFSuperResolutionPipeline.from_pretrained(
            model_key_2, text_encoder=None, variant="fp16", torch_dtype=torch.float16
        )
        self.stage_2_pipe.safety_checker = None
        self.stage_2_pipe.watermarker = None

        self.stage_3_pipe = StableDiffusionUpscalePipeline.from_pretrained(
    ...
    def get_text_embeds(self, prompts, negative_prompts):
        ...
        prompt_embeds = self.text_encoder(
            text_inputs.input_ids.to(mm.gpu), text_inputs.attention_mask.to(mm.gpu)
        )[0]

        negative_prompts = [
            negative_prompt.lower().strip() for negative_prompt in negative_prompts
        ]
        ...
        uncond_embeds = self.text_encoder(
            uncond_inputs.input_ids.to(mm.gpu), uncond_inputs.attention_mask.to(mm.gpu)
        )[0]
    ...
    def ddpm_update(self, intermediate_images, images, noise, t, alpha_prod_t_prev):
        ...
        pred_prev_sample = (
            pred_original_sample_coeff * images
            + current_sample_coeff * intermediate_images
        )
    ...
    def __call__(
        ...
        for counter, t in enumerate(tqdm(timesteps)):
            ...
            images = torch.stack(  # [N, C, H, W]  N: the number of levels
                [zoom_stack.render_image(i) for i in range(num_levels)]
            ).to(device=mm.gpu, dtype=torch.float16)
            zoom_stack.sample_noise(zoom_stack.E.shape)
            noise = torch.stack(
                [zoom_stack.render_noise(i) for i in range(num_levels)]
            ).to(device=mm.gpu, dtype=torch.float16)
            ...
            intermediate_images = self.ddpm_update(
                intermediate_images, images, noise, t, alpha_prod_t_prev
            )

            # unet forward: \hat{e}_t-1 = (1+w)*unet(z_t-1, t-1, y) - w*unet(z_t-1, t-1)
            noise_pred = self.unet_forward(  # CFG
                intermediate_images, t, text_embeds, guidance_scale
            )

            # update images (epsilon): \hat{x}_t-1 = (z_t-1 - sigma_t-1 * \hat{e}_t-1) / alpha_prod_t-1
            denoised_images = (
                intermediate_images - (1 - alpha_prod_t_prev) ** (0.5) * noise_pred
            ...
            if self.scheduler.config.thresholding:
                ...
                denoised_images = denoised_images.clamp(
            ...
            if photograph is not None and counter < num_inference_steps * 0.8:
                with torch.enable_grad():
                    denoised_images_copy = (
                        denoised_images.clone()
                    ...
                    for _ in range(5):
                        optimizer.zero_grad()
                        loss = self.photograph_loss(
                            zoom_stack, denoised_images_copy, photograph
                        )
                        loss.backward()
                        optimizer.step()
                    denoised_images = denoised_images_copy.detach().to(
                        dtype=torch.float16
                    )
        ...
        # Process in batches of batch_size to avoid OOM
        batch_size = 1
        upscaled_images = []
