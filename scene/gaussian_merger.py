...
class GaussianMerger:
    ...
    def init_from_pcd(self, points, colors):
        ...
        print(f"Adding {points.shape[0]} points into existing {self.gaussians._xyz.shape[0]} gaussians")
        ...
        opacities = inverse_sigmoid(
            0.1
            * torch.ones(
                (points.shape[0], 1), dtype=torch.float, device="cuda"
            )
        )
        return {'xyz': points, 'features_dc': features_dc, 'features_rest': features_rest, 'scaling': scales, 'rotation': rots, 'opacity': opacities}
    
    def merge_gaussians(self, new_gaussian_attrs: dict, opt: GSParams):
        gs = self.gaussians
        xyz = torch.cat([gs._xyz, new_gaussian_attrs['xyz']], dim=0)
        features_dc = torch.cat([gs._features_dc, new_gaussian_attrs['features_dc']], dim=0)
        features_rest = torch.cat([gs._features_rest, new_gaussian_attrs['features_rest']], dim=0)
