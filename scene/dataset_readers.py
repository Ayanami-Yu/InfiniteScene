...
def fetchPly(path):
    ...
    idx = np.random.choice(
        len(vertices["x"]), size=(min(len(vertices["x"]), 100_000),), replace=False
    )
    positions = (
        np.vstack([vertices["x"][idx], vertices["y"][idx], vertices["z"][idx]]).T
        if "x" in vertices
        else None
    )
    colors = (
        np.vstack(
            [vertices["red"][idx], vertices["green"][idx], vertices["blue"][idx]]
        ).T
        / 255.0
        if "red" in vertices
        else None
    ...
    normals = (
        np.vstack([vertices["nx"][idx], vertices["ny"][idx], vertices["nz"][idx]]).T
        if "nx" in vertices
        else None
    )
...
def readColmapSceneInfo(path, images, eval, preset=None, llffhold=8):
    ...
    if eval:
        ...
        train_cam_infos = [
            c for idx, c in enumerate(cam_infos) if idx % 5 == 2 or idx % 5 == 0
        ]
        test_cam_infos = [
            c for idx, c in enumerate(cam_infos) if not (idx % 5 == 2 or idx % 5 == 0)
        ]
    else:
    ...
    if preset:
        preset_cam_infos = readCamerasFromPreset(
            "/home/chung/workspace/gaussian-splatting/poses_supplementary",
            f"{preset}.json",
        )
    else:
...
def readCamerasFromPreset(path, transformsfile):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            # get the world-to-camera transform and set R, T
            w2c = np.linalg.inv(
                np.concatenate((c2w, np.array([0, 0, 0, 1]).reshape(1, 4)), axis=0)
            )
            R = np.transpose(
                w2c[:3, :3]
            )  # R is stored transposed due to 'glm' in CUDA code
...
def loadCamerasFromData(traindata, white_background):  # only used in readDtaInfo
    cameras = []
    ...
    for idx, frame in enumerate(frames):
        ...
        c2w = np.array(frame["transform_matrix"])
        # change from OpenGL/Blender camera axes (Y up, Z back) to COLMAP (Y down, Z forward)
        # NOTE This operation is very misleading. In fact, in generate_pcd, the y and z axes are reversed right before being added into traindata, so the operation here is just recovering the axis directions as originally generated by get_pcdGenPoses. This note also applies to loadCameraPreset.
        c2w[:3, 1:3] *= -1
...
def loadCameraPreset(traindata, presetdata):
    ...
    for camkey in presetdata:
        ...
        for frame in presetdata[camkey]["frames"]:
            ...
            c2w = np.array(frame["transform_matrix"])
            # change from OpenGL/Blender camera axes (Y up, Z back) to COLMAP (Y down, Z forward)
            # caution: see the note in loadCamerasFromData
            c2w[:3, 1:3] *= -1
